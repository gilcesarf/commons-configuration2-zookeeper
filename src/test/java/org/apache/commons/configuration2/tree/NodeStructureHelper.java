...
public class NodeStructureHelper {
    ...
    private static final String[][] WORKS = {
            {
                    "Troilus and Cressida", "The Tempest", "A Midsummer Night's Dream"
            }, {
                    "Ilias"
            }, {
                    "Ilium", "Hyperion"
    ...
    public static String appendPath(String path, String component) {
        StringBuilder buf = new StringBuilder(StringUtils.length(path) + StringUtils.length(component) + 1);
     ...
     * {@code path/to/node}. If there are multiple sibling nodes with the same name, a numerical index can be specified
     * in parenthesis.
     *
     * @param root
     *            the root node
     * @param key
     *            the key to the desired node
     ...
     * @throws NoSuchElementException
     *             if the desired node cannot be found
     */
    ...
    /**
     * Evaluates the given key and finds the corresponding child node of the root node of the specified
     * {@code NodeHandler} object. This is a convenience method that works like the method with the same name, but
     * obtains the root node from the given handler object.
     *
     * @param handler
     *            the {@code NodeHandler} object
     * @param key
     *            the key to the desired node
     * @return the found target node
     * @throws NoSuchElementException
     ...
     *
     * @param path
     *            an array with the expected node names on the path
    ...
    /**
     * Helper method for creating a field node with its children. Nodes of this type are used within the tables tree.
     * They define a single column of a table.
     *
     * @param name
    ...
    public static NodeKeyResolver<ImmutableNode> createResolverMock() {
        ...
        NodeKeyResolver<ImmutableNode> mock = EasyMock.createMock(NodeKeyResolver.class);
        return mock;
    }

    /**
     * Prepares a mock for a resolver to expect arbitrary resolve operations. These operations are implemented on top of
     * a default expression engine.
     *
     * @param resolver
     *            the mock resolver
     */
    ...
    public static void expectResolveKeyForQueries(NodeKeyResolver<ImmutableNode> resolver) {
        EasyMock.expect(resolver.resolveKey(EasyMock.anyObject(ImmutableNode.class),
                EasyMock.anyObject(String.class),
                (NodeHandler<ImmutableNode>) EasyMock.anyObject(NoHandler.class)))
                .andAnswer(new IAnswer<List<QueryResult<ImmutableNode>>>() {
                    @Override
                    public List<QueryResult<ImmutableNode>> answer() throws Throwable {
                        ImmutableNode root = (ImmutableNode) EasyMock.getCurrentArguments()[0];
                        String key = (String) EasyMock.getCurrentArguments()[1];
                        NodeHandler<ImmutableNode> handler =
                                (NodeHandler<ImmutableNode>) EasyMock.getCurrentArguments()[2];
     ...
     *
     * @param resolver
     *            the {@code NodeKeyResolver} mock
     */
    public static void expectResolveAddKeys(NodeKeyResolver<ImmutableNode> resolver) {
        EasyMock.expect(resolver.resolveAddKey(EasyMock.anyObject(ImmutableNode.class),
                EasyMock.anyString(),
                EasyMock.anyObject(TreeData.class))).andAnswer(new IAnswer<NodeAddData<ImmutableNode>>() {
                    @Override
    ...
    /**
     * Creates as tree with database table data based on the passed in arrays of table names and fields for tables.
     * Works like the method without parameters, but allows defining the data of the structure.
     *
     * @param tables
     *            an array with the names of the tables
     * @param fields
     *            an array with the fields of the single tables
    ...
    /**
     * Returns a clone of the array with the table names. This is useful if a slightly different tree structure should
     * be created.
     *
    ...
    /**
     * Returns a clone of the array with the table fields. This is useful if a slightly different tree structure should
     * be created.
     *
    ...
    /**
     * Creates a tree with a root node whose children are the test authors. Each other has his works as child nodes.
     * Each work has its personae as children.
     *
     ...
     * attribute pointing to the author who invented this person. There is a single child node for the associated work
     * which has again a child and an attribute.
     *
    ...
    private static ImmutableNode createPersonaeTree() {
        ...
        for (int author = 0; author < AUTHORS.length; author++) {
            for (int work = 0; work < WORKS[author].length; work++) {
                for (String person : PERSONAE[author][work]) {
                    ImmutableNode orgValue = new ImmutableNode.Builder().name(ELEM_ORG_VALUE)
                            .value("yes")
                            .addAttribute(ATTR_TESTED, Boolean.FALSE)
                            .create();
                    ImmutableNode workNode =
                            new ImmutableNode.Builder(1).name(WORKS[author][work]).addChild(orgValue).create();
                    ImmutableNode personNode = new ImmutableNode.Builder(1).name(person)
     ...
     * @param components
     *            the array with the components of the node key
     * @param currentIdx
     *            the index of the current path component
     * @return the found target node
     * @throws NoSuchElementException
     *             if the desired node cannot be found
     */
